...
function analyzePDFText(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const keyword of keywords) {
      ...
      if (regex.test(line)) {
        ...
        // Group accounts with the same keyword
        if (!negativeAccounts[keyword]) {
          negativeAccounts[keyword] = [];
        }
        negativeAccounts[keyword].push(account);

        // Move to the next line after finding a match
        break;
      }
    }
  }
  ...
  const result = [];
  for (const [keyword, accounts] of Object.entries(negativeAccounts)) {
    result.push({ keyword, accounts });
  }
