  ...
  function processInquiryBlock(lines) {
    ...
    lines.forEach((line, index) => {
      ...
      let normalizedLine = cleanedLine
        .replace(/(\d{2}\/\d{2}\/\d{4})/g, ' $1 ') // Ensure dates are kept together
        .replace(/(\d{4}-\d{2}-\d{2})/g, ' $1 ') // Ensure dates are kept together for different date format
        .replace(/([a-z])([A-Z])/g, '$1 $2') // Add space between lowercase and uppercase letters (e.g., "AbcofAmerica" to "Abc of America")
      ...
      // If the date is found and the parts array has at least 4 parts, create an inquiry object
      if (dateFound && parts.length >= 4) {
        // Handle creditor name correctly
        const creditor = parts.slice(0, dateIndex - 1).join(' '); // Combine parts before the type of business into the creditor
        const typeOfBusiness = parts[dateIndex - 1];
