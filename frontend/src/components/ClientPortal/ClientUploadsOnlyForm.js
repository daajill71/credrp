...
const ClientUploadsOnlyForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const docupData = new FormData();

      for (const fieldName in fileUploads) {
        const file = fileUploads[fieldName];
        if (file) {
          docupData.append(fieldName, file);
        }
      }

      const docupResponse = await axios.post('http://localhost:5000/docup/uploaded', docupData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });

      if (docupResponse.status === 201) {
        setSuccessMessage('Documents submitted successfully');
        setTimeout(() => setSuccessMessage(null), 5000);
        console.log('Documents uploaded successfully:', docupResponse.data);
      } else {
        console.error('Error uploading documents:', docupResponse.status);
...
export default ClientUploadsOnlyForm;
