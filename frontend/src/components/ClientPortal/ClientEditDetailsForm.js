...
const ClientEditDetailsForm = () => {
  ...
  useEffect(() => {
    const fetchClientDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/get-client/${_id}`); // Use _id from URL
        const { data } = response;
        setFormData(data);
        setLoading(false);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.put(`http://localhost:5000/edit-client/${_id}`, formData); // Use _id from URL
